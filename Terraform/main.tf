...
# getting iam role from exiting roles named eks-role-jenkins-new-repo
...
# getting iam role from exiting roles named eks-node-role-jenkins-new-repo
...
resource "aws_iam_role_policy_attachment" "efs_access_policy_attachment" {
  ...
  role       = data.aws_iam_role.eks-node-role.name
}
